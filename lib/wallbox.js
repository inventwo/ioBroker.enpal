...
function findOnclickHandlers(data, log) {
	...
	const f0 = data.readUInt32LE(footerOff);
	const f1 = data.readUInt32LE(footerOff + 4);
	const f2 = data.readUInt32LE(footerOff + 8);
	const f3 = data.readUInt32LE(footerOff + 12);
	const f4 = data.readUInt32LE(footerOff + 16);
	if (log) {
		log.debug(
			`[Wallbox] RenderBatch footer: updatedComponents=${f0} refFrames=${f1} disposedComp=${f2} disposedHandlers=${f3} strings=${f4}`,
		);
	}
	const refFramesOff = f1;
	if (refFramesOff === 0 || refFramesOff >= data.length - 4) {
		if (log) {
			log.debug(`[Wallbox] findOnclickHandlers: invalid refFramesOff=${refFramesOff}`);
		}
		return [];
	}
	const frameCount = data.readUInt32LE(refFramesOff);
	if (log) {
		log.debug(`[Wallbox] RenderBatch frameCount=${frameCount}`);
	}
	const handlers = [];
...
class WallboxBlazorClient {
		...
		for (const msg of msgs) {
			...
			if (msgType === 3) {
				...
				if (invId === '0' && this._startCircuitResolve) {
					// StartCircuit completion: resultKind 3 = value, result should be true
					this._startCircuitResolve(resultKind === 3 ? result === true : resultKind === 2);
				}
				if (resultKind === 1) {
	...
	_processRenderBatch(data) {
		...
		if (handlers.length > 0) {
			// The last N handlers (up to 6) are the wallbox buttons in BUTTON_ORDER sequence.
			// Earlier handlers belong to nav/header elements.
			const take = Math.min(handlers.length, BUTTON_ORDER.length);
			const wallboxHandlers = handlers.slice(-take);
