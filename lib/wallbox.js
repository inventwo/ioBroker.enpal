...
class WallboxBlazorClient {
	...
	async fetchStatus() {
		try {
			const html = await this._httpGet('/wallbox');
			// Log a section likely to contain the rendered component content
			const bodyIdx = html.indexOf('<body');
			const snippet = bodyIdx >= 0 ? html.substring(bodyIdx, bodyIdx + 3000) : html.substring(0, 3000);
			this._log.debug(`[Wallbox] HTML body snippet (3000 chars): ${snippet}`);
