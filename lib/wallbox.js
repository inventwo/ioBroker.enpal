...
function findOnclickHandlers(data, log) {
	...
	// PRIMARY SCAN: Blazor .NET 8 attribute frame layout (24 bytes):
	//   offset  0: FrameType (int32)          = 3 for Attribute
	//   offset  4: Sequence (int32)           = call-sequence within component
	//   offset  8: EventHandlerMarker (uint32) = 0xffffffff marks an event-handler attribute
	//   offset 12: EventHandlerId (uint32)    = 0 = JS-only, >0 = registered .NET onclick handler
	//   offset 16: (reserved / zero)
	//   offset 20: EventNameStringIdx (uint32) = string-table index for event name ("onclick" etc.)
	//
	// We look for frames where offset8=0xffffffff AND offset12>0 — those are the actual
	// .NET server-side onclick handlers the wallbox buttons register.
	// Frames with offset12=0 are JS-only events (hover, focus, blur) with no server handler.
	for (let fi = 0; fi < 5; fi++) {
		...
		for (const frameSize of [24, 28, 20]) {
			if (refFramesOff + 4 + frameCount * frameSize > data.length + frameSize) {
				continue;
			}
			const handlers = [];
			// Also track ALL sentinel frames to log the complete picture
			const allSentinels = [];
			...
			for (let i = 0; i < frameCount; i++) {
				...
				if (data.readInt32LE(pos) === BLAZOR_FRAME_TYPE_ATTRIBUTE) {
					const sentinel = data.readUInt32LE(pos + 8);
					if (sentinel === 0xffffffff) {
						const handlerId = data.readUInt32LE(pos + 12);
						const seq = data.readUInt32LE(pos + 4);
						const evtName = data.readUInt32LE(pos + 20);
						allSentinels.push({ seq, handlerId, evtName });
						if (handlerId > 0 && handlerId < 10000) {
			...
			if (allSentinels.length > 0) {
				if (log) {
					log.info(
						`[Wallbox] Sentinel frames (fi=${fi} fz=${frameSize}): ` +
							allSentinels.map((s) => `seq${s.seq}→hId=${s.handlerId}`).join(', '),
					);
					if (handlers.length > 0) {
						log.info(
							`[Wallbox] PRIMARY scan OK: ${handlers.length} .NET onclick handler(s): [${handlers.join(',')}]`,
						);
					} else {
						log.info(
							`[Wallbox] PRIMARY scan: all sentinel frames have handlerId=0 (JS-only events). ` +
								`No .NET server-side handlers found via offset-12 method.`,
						);
					}
				}
				if (handlers.length > 0) {
				...
				// Found sentinel frames but all handlerId=0 — stop trying other frame sizes
				// for this footer value; the frame layout is confirmed but there are no handlers.
				break;
			}
		}
	}

	// FALLBACK SCAN (legacy): read offset 20 as potential handler ID.
	// NOTE: This typically returns string-table indices, NOT real event-handler IDs.
	// It is kept only as a last resort for older Blazor versions where the layout differs.
	...
	if (best.handlers.length > 0) {
		if (log) {
			log.info(
				`[Wallbox] FALLBACK scan OK: footerIdx=${best.fi} frameSize=${best.frameSize} handlerOff=${best.handlerOffset} → ${best.handlers.length} handler(s): [${best.handlers.join(',')}]`,
			);
		}
...
class WallboxBlazorClient {
	...
	async fetchStatus() {
		try {
			const html = await this._httpGet('/wallbox');
			// Log snippet once to diagnose HTML structure when mode/status have never been found
			if (!this._htmlSnippetLogged && !this._mode && !this._status) {
				this._htmlSnippetLogged = true;
				this._log.info(`[Wallbox] HTML snippet (first 800 chars): ${html.substring(0, 800)}`);
			}
	...
	_processRenderBatch(data) {
		...
		// Exception: log batches for 3 s after a click to see if the UI actually changed.
		if (this._connected) {
