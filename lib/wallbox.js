...
class WallboxBlazorClient {
	...
	async connect() {
		...
		try {
			...
			// 8. Wait up to 5 s for all render batches to arrive, but exit early the
			// moment all 6 handlers are known. The Blazor Server circuit closes after
			// ~3 s of inactivity, so we must click well before that deadline.
			// A minimum of 200 ms (2 iterations) gives time for late batches.
			for (let i = 0; i < 50; i++) {
				await sleep(100);
