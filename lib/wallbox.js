	...
	for (const [frameSize, handlerOffset] of layouts) {
		for (let fi = 0; fi < 5; fi++) {
			...
			if (handlers.length > 0) {
				if (log) {
					log.info(
						`[Wallbox] Frame scan OK: footerIdx=${fi} refOff=${refFramesOff} frameSize=${frameSize} handlerOff=${handlerOffset} → ${handlers.length} handler(s): [${handlers.join(',')}]`,
					);
				}
				return handlers;
			}
...
class WallboxBlazorClient {
	...
	async connect() {
		...
		try {
			...
			// 7. For .NET 8 Blazor: tell server which components to render
			await sleep(200);
			await this._sendUpdateRootComponents();
			this._log.info('[Wallbox] Step 6 OK: UpdateRootComponents sent');

			// 8. Wait up to 5 s for all render batches to arrive.
			// In .NET 8 Blazor, mode-button handlers come in batches triggered by
			// UpdateRootComponents — we MUST NOT break early on the first batch.
			// Always wait at least 3 s, then allow early exit only if all 6 buttons found.
			for (let i = 0; i < 50; i++) {
				await sleep(100);
				if (i >= 30 && Object.keys(this._buttonHandlers).length >= BUTTON_ORDER.length) {
					break;
				}
			}
			const handlerCount = Object.keys(this._buttonHandlers).length;
			this._log.info(
				`[Wallbox] Step 7: received ${handlerCount} button handler(s): ${JSON.stringify(this._buttonHandlers)}`,
			);
			if (handlerCount === 0) {
	...
	async _sendEndInvokeJs(taskId, fnName) {
		// EndInvokeJSFromDotNet: args = [asyncHandle, succeeded, argsOrError]
		// argsOrError must be a non-null JSON string — the Blazor 8 circuit dispatcher
		// parses it as the packed tuple [asyncHandle, succeeded, resultValue].
		// Sending bare "null" causes "Invalid interop arguments" and a circuit crash.
		let innerVal = 'null';
		if (fnName === 'mudpopoverHelper.countProviders') {
			// MudBlazor expects an int result (number of registered popover providers).
			// Returning 0 lets it complete its initialisation and trigger a state update
			// that renders the mode-selection buttons with real event-handler IDs.
			innerVal = '0';
		}
		const argsOrError = `[${taskId},true,${innerVal}]`;
		await this._sendMessage([1, {}, null, 'EndInvokeJSFromDotNet', [taskId, true, argsOrError]]);
	}
	...
	async _handleMessages(buf) {
		...
		for (const msg of msgs) {
			...
			if (target === 'JS.RenderBatch') {
				...
				if (jsArgsRaw && typeof jsArgsRaw === 'string' && jsArgsRaw.includes('"__dotNetObject"')) {
					try {
				...
				if (taskId != null) {
					await this._sendEndInvokeJs(taskId, jsFnName);
				}
	...
	_processRenderBatch(data) {
		...
		// One-time hex dump of the reference-frames section for the first large batch.
		if (!this._framesDumped && data.length >= 500) {
			...
			for (let fi = 0; fi < 5; fi++) {
				...
				const dumpBytes = Math.min(4 + fc * 24, 4 + 24 * 24, data.length - refOff);
				this._log.info(
					`[Wallbox] FRAMES-DUMP fi=${fi} refOff=${refOff} fc=${fc}: ${data.slice(refOff, refOff + dumpBytes).toString('hex')}`,
				);
			}
		}
