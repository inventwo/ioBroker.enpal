...
class WallboxBlazorClient {
	...
	async clickButton(button) {
		...
		// eventArgsType must be 'mouse' (not 'click') to match the server's
		// EventHandlerCallerInfo.EventArgsType deserialization.
		...
		const argsJson = JSON.stringify([eventDescriptor, JSON.stringify(eventArgs)]);
		const clickMsg = [
			1,
			{},
			null,
