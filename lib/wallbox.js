...
class WallboxBlazorClient {
	...
	async clickButton(button) {
		...
		// Blazor Server uses DispatchBrowserEvent directly on the hub (fire-and-forget,
		// no EndInvokeDotNet response). BeginInvokeDotNetFromJS/DispatchEventAsync is the
		// Blazor Web App variant; the Enpal Box uses pure Blazor Server.
		const clickMsg = [
			1,
			{},
			null,
			...
			[
				{ browserRendererId: 0, eventHandlerId: handlerId, eventArgsType: 'mouse', eventFieldInfo: null },
				{
					type: 'click',
					detail: 1,
					screenX: 100,
		...
		this._log.info(`[Wallbox] Sending DispatchBrowserEvent: button=${button} handlerId=${handlerId}`);
		await this._sendMessage(clickMsg);
		this._log.info(`[Wallbox] DispatchBrowserEvent sent for button="${button}" — waiting 1 s for server response`);
		// Give the server 1 s to process the click and send a render-batch update.
		await sleep(1000);
		this._log.info(`[Wallbox] Click complete for button="${button}"`);
	}
