...
function extractStatusFromHtml(html) {
	let mode = null,
		status = null;
	const modeMatch = html.match(/Mode\s+(Eco|Full|Solar|Smart)/i);
	if (modeMatch) {
		mode = modeMatch[1].charAt(0).toUpperCase() + modeMatch[1].slice(1).toLowerCase();
	}
...
class WallboxBlazorClient {
	...
	async fetchStatus() {
		try {
			...
			this._log.debug(`[Wallbox] Status fetch failed: ${err.message}`);
			return { mode: this._mode, status: this._status };
		}
	}
