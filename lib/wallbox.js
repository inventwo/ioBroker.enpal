...
class WallboxBlazorClient {
	...
	async _sendEndInvokeJs(taskId, fnName) {
		// EndInvokeJSFromDotNet: args = [taskId, success, jsonEncodedResult]
		// The third argument must be the JSON-encoded return value of the JS function.
		// Sending the wrong type causes a C# deserialization error that silently
		// prevents MudBlazor from completing its initialisation (and thereby
		// blocks the server from rendering mode-button event-handler IDs).
		let jsonResult = 'null'; // void / null return
		if (fnName === 'mudpopoverHelper.countProviders') {
			// Server expects an int — return 0 (no providers registered yet)
			jsonResult = '0';
		}
		await this._sendMessage([1, {}, null, 'EndInvokeJSFromDotNet', [taskId, true, jsonResult]]);
	}
